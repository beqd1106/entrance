...
function viewHome() {
  app.appendChild(h('section.hero',
    h('div.wrap',
      ...
      h('p.reveal-2', { style: { marginTop: '16px', fontSize: '16px' } },
        'JANDOORは、全国の雀荘がそれぞれの「オンライン支店」を持つためのプラットフォームです。'
  ...
  wrap.appendChild(h('div.cta-band',
    h('div',
      ...
      h('p', { text: 'ルールを読むだけでは不安は消えません。JANDOORは、その店のルールで実際に1半荘打てるところまでを入口にします。' })),
...
function footer() {
  return h('footer.footer', h('div.wrap',
    h('div', { text: 'JANDOOR デモ版 ／ 実在店舗のハウスルールを転載していない架空データです。' }),
