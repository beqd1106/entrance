...
│   └── BundleSchemeHandler.swift  同梱資産を houserule://app/... で配信
├── Resources/
...
- `file://` ではES Modulesの読み込みがブロックされるため、**独自スキーム（`houserule://app/`）**で配信しています（Capacitorと同じ考え方）。
...
git init
git add .
git commit -m "Houserule 初回コミット"
gh repo create beqd1106/houserule --private --source=. --push
...
3. XcodeGenで `Houserule.xcodeproj` を生成
