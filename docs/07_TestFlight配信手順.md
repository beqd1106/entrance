...
│   └── BundleSchemeHandler.swift  同梱資産を jandoor://app/... で配信
├── Resources/
...
- `file://` ではES Modulesの読み込みがブロックされるため、**独自スキーム（`jandoor://app/`）**で配信しています（Capacitorと同じ考え方）。
...
```bash
# リポジトリを用意（初回のみ）
cd C:\Users\user\Downloads\JANDOOR
git init
git add .
git commit -m "JANDOOR 初回コミット"
gh repo create beqd1106/jandoor --private --source=. --push
...
3. XcodeGenで `JANDOOR.xcodeproj` を生成
